...
@staticmethod
def load_calibration_data(include_fake=False, include_hist=False, num_hist=10, holdouts=None, load_prof=None):
    ...
    for filename in files:
        ...
        if is_fake and not include_fake:
            continue
        if is_hist and not include_hist:
            continue
        filtered_files.append(filename)

    # if hist, pre-select which files to load before loading them
    if include_hist and holdouts:
        ...
        selected_hist = list(holdouts)  # always include holdouts
        already = set(holdouts)
        for backend, fnames in by_backend.items():
            remaining = [f for f in fnames if f not in already]
            selected_hist.extend(random.sample(remaining, min(num_hist, len(remaining))))
    ...
    for filename in tqdm(filtered_files, desc="Loading calibration profiles"):
        ...
        with open(filepath) as f:
            data = json.load(f)
        prof = build_backend(data, filename)
        if prof is not None:
...
def build_backend(data, filename):
    ...
    try:
        ...
        return
    csc = compute_csc_from_props(props)
    return ({
            "filename": filename,
...
def CSC(qubits):
    ...
    """
    return sum(GSC(gate_errors, readout_error) for gate_errors, readout_error in qubits) / len(qubits)
